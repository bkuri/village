...
class TestApproveRejectCLI:
    """Tests for --approve, --approve-all, --reject CLI options."""

    def test_approve_pending_task(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --approve transitions PENDING_APPROVAL to QUEUED."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
        ...
        sm = TaskStateMachine(config)
        sm.initialize_state("bd-a3f8", TaskState.QUEUED)
        sm.transition("bd-a3f8", TaskState.PENDING_APPROVAL)

        with patch("village.cli.work.get_config", return_value=config):
            result = runner.invoke(village, ["queue", "--approve", "bd-a3f8"])

        assert result.exit_code == 0
        assert "Approved task bd-a3f8" in result.output
        state = sm.get_state("bd-a3f8")
        assert state == TaskState.QUEUED

    def test_approve_non_pending_task(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --approve on non-pending task returns error."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
    ...
    def test_reject_pending_task(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --reject transitions PENDING_APPROVAL to FAILED."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
        ...
        with patch("village.cli.work.get_config", return_value=config):
            result = runner.invoke(village, ["queue", "--reject", "bd-a3f8"])

        assert result.exit_code == 0
        assert "Rejected task bd-a3f8" in result.output
        state = sm.get_state("bd-a3f8")
        assert state == TaskState.FAILED

    def test_reject_non_pending_task(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --reject on non-pending task returns error."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
    ...
    def test_approve_all_pending_tasks(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --approve-all approves all PENDING_APPROVAL tasks."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
        ...
        sm.initialize_state("bd-a3f8", TaskState.QUEUED)
        sm.transition("bd-a3f8", TaskState.PENDING_APPROVAL)
        sm.initialize_state("bd-b7d2", TaskState.QUEUED)
        sm.transition("bd-b7d2", TaskState.PENDING_APPROVAL)

        with patch("village.cli.work.get_config", return_value=config):
            result = runner.invoke(village, ["queue", "--approve-all"])

        assert result.exit_code == 0
        assert "Approved 2 task(s)" in result.output
        assert sm.get_state("bd-a3f8") == TaskState.QUEUED
        assert sm.get_state("bd-b7d2") == TaskState.QUEUED

    def test_approve_all_skips_non_pending(self, runner: click.testing.CliRunner, tmp_path: Path):
        """Test --approve-all only approves PENDING_APPROVAL tasks."""
        tmp_path.mkdir(parents=True, exist_ok=True)
        subprocess.run(["git", "init"], cwd=tmp_path, check=True)
        config = Config(
            git_root=tmp_path,
        ...
        assert result.exit_code == 0
        assert "Approved 1 task(s)" in result.output


class TestCIModePendingApproval:
    """Tests for CI mode error on pending approvals."""

    def test_ci_mode_error_on_pending_approval(self, runner: click.testing.CliRunner, mock_config: Config):
        """Test CI mode errors when tasks pending approval."""
        blocked = [
            QueueTask(
                task_id="bd-a3f8",
                agent="build",
        ...
        with patch("village.cli.work.generate_queue_plan", return_value=plan):
            with patch("village.cli.work.get_config", return_value=mock_config):
                with patch.dict("os.environ", {"VILLAGE_CI_MODE": "true"}):
                    result = runner.invoke(village, ["queue", "--plan"])

        assert result.exit_code == 1
        assert "pending approval in CI mode" in result.output

    def test_ci_mode_no_error_without_pending(self, runner: click.testing.CliRunner, mock_config: Config):
        """Test CI mode succeeds when no pending approvals."""
        plan = QueuePlan(
            ready_tasks=[],
            available_tasks=[],
            blocked_tasks=[],
